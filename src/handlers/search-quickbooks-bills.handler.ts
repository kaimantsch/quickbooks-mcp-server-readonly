 ...
 */
export async function searchQuickbooksBills(criteria: QuickbooksSearchCriteriaInput = {}): Promise<ToolResponse<any[]>> {
  try {
    ...
    return new Promise((resolve) => {
      (quickbooks as any).findBills(normalizedCriteria as any, (err: any, bills: any) => {
        if (err) {
          resolve({
            result: null,
            isError: true,
